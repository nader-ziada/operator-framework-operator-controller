...
func Test_PreAuthorizer_Integration(t *testing.T) {
	...
	newDummyGenerator := func(t *testing.T) applier.ClusterObjectSetGenerator {
		...
		m := mockapplier.NewMockClusterObjectSetGenerator(ctrl)
		m.EXPECT().GenerateRevision(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
			func(ctx context.Context, bundleFS fs.FS, ext *ocv1.ClusterExtension, objectLabels, revisionAnnotation map[string]string, nsConfig *applier.NamespaceConfig) (*ocv1ac.ClusterObjectSetApplyConfiguration, error) {
				return ocv1ac.ClusterObjectSet("").
					WithSpec(ocv1ac.ClusterObjectSetSpec().
						WithPhases(
							ocv1ac.ClusterObjectSetPhase().
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			completed, status, err := boxcutter.Apply(t.Context(), dummyBundleFs, ext, nil, revisionAnnotations, nil)
			if tc.validate != nil {
				tc.validate(t, err)
			}
			_ = completed
